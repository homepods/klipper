...
class TemperatureProbe(probe.PrinterProbe):
    def __init__(self, config, endstop):
        probe.PrinterProbe.__init__(self, config, endstop)
        self.get_status = self.mcu_probe.get_status
...
class TemperatureProbeEndstopWrapper(probe.ProbeEndstopWrapper):
    def __init__(self, config):
        ...
        for s in ["thermistor ", "adc_temperature "]:
            if config.has_section(s + sensor_type):
                self.printer.load_object(config, s + sensor_type)
                break
        heaters = self.printer.load_object(config, 'heaters')
        self.sensor = heaters.setup_sensor(config)
    ...
    def get_status(self, eventtime):
        current_offset = self.get_temperature_offset()
        return {
    ...
    def _wait_for_temp(self, gcmd, timeout, is_warming):
        reactor = self.printer.get_reactor()
        toolhead = self.printer.lookup_object('toolhead')
        eventtime = reactor.monotonic()
    ...
    def cmd_PROBE_WAIT(self, gcmd):
        self.target = gcmd.get_float('TARGET', 35., above=0.)
        timeout = gcmd.get_int('TIMEOUT', 0, minval=0)
        if self._check_heater_state():
            # either bed or extruder is on, probe is warming
            gcmd.respond_info("Waiting for probe to heat...")
            temp_acheived = self._wait_for_temp(gcmd, timeout, True)
        else:
            # both bed and extruder are off, probe is cooling
            gcmd.respond_info("Waiting for probe to cool...")
            temp_acheived = self._wait_for_temp(gcmd, timeout, False)
        if temp_acheived:
            gcmd.respond_info("Probe Temperature Achieved")
        else:
            gcmd.respond_info("Wait for Probe Temp Timed Out")
        self.target = 0.
